...
class get_model(nn.Module):
    def __init__(self,max_len = 100,d_model = 128,max_sub = 15):
        super(get_model, self).__init__()

        self.embed_layers = embed(2,7,128)
        self.dual_embed = dual_embed(128,8)

        self.sub_head = sub_head(max_sub)
    ...
    def forward(self,data):
        ...
        idx_odd = torch.arange(N/2)*2
        idx_even = idx_odd+1

        edge_feats = edge_feats[:,idx_odd.long(),:] +  edge_feats[:,idx_even.long(),:]

        edge_feats = self.sub_head(edge_feats)
        
        
        #para = self.head(node_next)
...
from data_loader_sub import rplan_dataset
